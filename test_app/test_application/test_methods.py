...
def create_models(n=2000, only=[1, 2, 3]):
    if 1 in only:
    ...
    if 2 in only:
        ...
        for i in range(n):
            ...
            m = Model2(name=random_hex(),
                       model1=Model1.objects.all()[t:t + 1][0],
    ...
    if 3 in only:
        ...
        for i in range(n):
            ...
            for j in range(rand.randint(1, 16)):
                t = rand.randint(2, k - 2)
                list_model1.append(Model1.objects.all()[t:t + 1][0])
            m.model1 = list_model1
            m.save()
...
def reference_get_test(n=10000):
    # warmer
    pk_list = [ m.pk for m in Model1.objects.all() ]
    for k in pk_list:
        m = Model1.objects.get(pk=k)
